...
public class Mario extends MovingObject{
	...
	public Mario(Mario other) { //Constructora por copia que devuelve una copia del Mario leído en la file.
		super(other);
	...
	public Mario parse(String[] objectDescription, GameWorld game) throws GameParseException, OffBoardException {
		GameObject go = super.parse(objectDescription, game); //parsea la posicion, el nombre y la acción -> puede lanzar excepciones de PositionParseException, ActionParseException (ambas subclases de GameParseException) y OffBoardException
		if (go!= null) {
			Mario mario = (Mario)go; //si el objeto no es nulo, es un Mario
			
			if (objectDescription.length > 4) {
				throw new ObjectParseException(Messages.COMMAND_ADDOBJECT_ERROR.formatted(String.join(" ", objectDescription))); //más parámetros de los necesarios
			}
			...
			if (objectDescription.length == 4) {
				...
				else throw new GameParseException(Messages.INVALID_MARIO_SIZE.formatted(String.join(" ", objectDescription))); //el nombre del tamaño es incorrecto
			}
			
			//si el tamaño es menor a 4 (no se especifica el tamaño) se pone por defecto
			return mario;
		}
		return null;
	}
