...
public class Game implements GameModel, GameStatus, GameWorld{
	...
	public void update() {
		remainingTime--;
		gameObjects.update();//Llamamos al GameObjectContainer para que actualice todos los objetos.
	}
	
		//ACCIONES DE MARIO
	@Override
	public void addAction(Action act){ //Lo llama ActionCommand para añadirle acciones a la lista de acciones de Mario
	...
	public GameObject addGameObject(String[] objWords) throws GameParseException, OffBoardException{
		//creamos un Mario temporal en el caso que no exista (inItLevelBlank)
		Mario tempMario = new Mario();
		
		//probar si el usuario esta añadiendo un Mario ya que su adición es diferente
		Mario newMario = tempMario.parse(objWords, this);
		GameObject newObject;
		
		if (newMario != null) { //como el parse funciona, estamos añadiendo un Mario
			...
			//si no está añadiendo un Mario, llamamos al parse de los demás objetos.
		...
		return null; //no se llega a ejecutar el que devuelva null, antes salta una excepcion
	}
	private void add(int i, GameObject obj) {
		gameObjects.add(i, obj);
	}
	private void changeMario(Mario newMario) { //si hay un mario, se tiene que eliminar de la lista de objetos para reasignarlo de nuevo
		if(this.mario != null) { //si ya existe un Mario en el juego, lo quitamos
	...
	public void load(String fileName) throws GameLoadException {
		...
		this.fileLoader = loadGame; //el atributo del fileLoader pasa a ser el fichero parseado
	...
	private void loadGame(GameConfiguration config) {//no carga los puntos ni las vidas ya que no se actualizan si se resetea el juego
